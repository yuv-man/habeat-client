...
export function ChallengeCard({ challenge, onClaim, compact = false, className }: ChallengeCardProps) {
  ...
  // Difficulty badge colors - habit-focused labels
  ...
  // Map difficulty to habit-building label
  const difficultyLabels: Record<string, string> = {
    starter: "3-day habit",
    building: "7-day habit",
    established: "14-day habit",
    easy: "3-day habit",
    medium: "7-day habit",
    hard: "14-day habit",
  };
    ...
    >
      ...
      <div className="mb-3">
        <div className="flex justify-between text-xs text-gray-500 mb-1">
          ...
          <span className="font-medium">{challenge.progress}/{challenge.target} days</span>
        </div>
        <div className="bg-gray-200 rounded-full h-2 overflow-hidden">
          <div
            className={cn(
              "h-full rounded-full transition-all duration-500",
              isCompleted ? "bg-gradient-to-r from-green-400 to-green-500" : "bg-gradient-to-r from-blue-400 to-indigo-500"
            )}
            style={{ width: `${progress}%` }}
          />
        </div>
