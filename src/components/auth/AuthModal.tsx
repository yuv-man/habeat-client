...
const AuthModal = ({ children, userData, onSuccess }: AuthModalProps) => {
  ...
  const handleOAuth = async (provider: string) => {
    try {
      ...
      toast({
        title: "OAuth Error",
        description: error instanceof Error ? error.message : "Failed to initiate OAuth authentication",
        variant: "destructive",
      });
      setIsLoading(false);
    }
      ...
      <DialogContent className="max-w-md">
        ...
        <Card className="border-0 shadow-none">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              {isLogin 
                ? "Sign in to your Habeat account" 
                : "Join Habeat and start your health journey"
              }
            </CardDescription>
          </CardHeader>
